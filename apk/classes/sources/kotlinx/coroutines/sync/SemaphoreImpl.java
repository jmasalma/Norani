...
public class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(Waiter waiter) {
        ...
        Function2 function2 = (KFunction) SemaphoreImpl$addAcquireToQueue$createNewSegment$1.INSTANCE;
        long access$getSEGMENT_SIZE$p = andIncrement / ((long) SemaphoreKt.SEGMENT_SIZE);
        loop0:
        while (true) {
            findSegmentInternal = ConcurrentLinkedListKt.findSegmentInternal(semaphoreSegment, access$getSEGMENT_SIZE$p, function2);
            if (SegmentOrClosed.m139isClosedimpl(findSegmentInternal)) {
                break;
            }
            Segment r8 = SegmentOrClosed.m137getSegmentimpl(findSegmentInternal);
            while (true) {
        ...
        SemaphoreSegment semaphoreSegment2 = (SemaphoreSegment) SegmentOrClosed.m137getSegmentimpl(findSegmentInternal);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        long access$getSEGMENT_SIZE$p = andIncrement / ((long) SemaphoreKt.SEGMENT_SIZE);
        Function2 function2 = (KFunction) SemaphoreImpl$tryResumeNextFromQueue$createNewSegment$1.INSTANCE;
        loop0:
        while (true) {
            findSegmentInternal = ConcurrentLinkedListKt.findSegmentInternal(semaphoreSegment, access$getSEGMENT_SIZE$p, function2);
            if (SegmentOrClosed.m139isClosedimpl(findSegmentInternal)) {
