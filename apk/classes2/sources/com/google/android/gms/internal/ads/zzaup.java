package com.google.android.gms.internal.ads;

import android.content.Context;
import com.google.common.util.concurrent.ListenableFuture;
import java.io.File;
import java.io.FileInputStream;
import java.io.IOException;
import java.security.MessageDigest;
import java.security.NoSuchAlgorithmException;
import java.util.concurrent.Executor;

/* compiled from: com.google.android.gms:play-services-ads@@24.5.0 */
public final class zzaup {
    private final ListenableFuture zza;
    private final zzavx zzb;
    private final zzasp zzc;

    public zzaup(Context context, Executor executor, zzasp zzasp, zzavx zzavx) {
        this.zzb = zzavx;
        this.zzc = zzasp;
        this.zza = zzgdn.zzj(new zzaun(this, context), executor);
    }

    private static String zzc(Context context) {
        FileInputStream fileInputStream;
        File file = new File(context.getPackageResourcePath());
        if (!file.exists() || !file.canRead()) {
            return "";
        }
        try {
            fileInputStream = new FileInputStream(file);
            byte[] bArr = new byte[16384];
            MessageDigest instance = MessageDigest.getInstance("SHA256");
            for (int read = fileInputStream.read(bArr); read != -1; read = fileInputStream.read(bArr)) {
                instance.update(bArr, 0, read);
            }
            zzgbd zzf = zzgbd.zzi().zzf();
            byte[] digest = instance.digest();
            String zzj = zzf.zzj(digest, 0, digest.length);
            fileInputStream.close();
            return zzj;
        } catch (IOException | UnsupportedOperationException | NoSuchAlgorithmException unused) {
            return "";
        } catch (Throwable th) {
            th.addSuppressed(th);
        }
        throw th;
    }

    /* access modifiers changed from: private */
    /* JADX WARNING: Can't wrap try/catch for region: R(2:33|34) */
    /* JADX WARNING: Code restructure failed: missing block: B:34:?, code lost:
        r1 = com.google.android.gms.internal.ads.zzgdn.zzh("");
     */
    /* JADX WARNING: Missing exception handler attribute for start block: B:33:0x00ba */
    /* JADX WARNING: Removed duplicated region for block: B:26:0x0055 A[SYNTHETIC, Splitter:B:26:0x0055] */
    /* JADX WARNING: Removed duplicated region for block: B:28:0x005a A[SYNTHETIC, Splitter:B:28:0x005a] */
    /* JADX WARNING: Removed duplicated region for block: B:38:0x00cb  */
    /* JADX WARNING: Removed duplicated region for block: B:44:0x00da A[SYNTHETIC, Splitter:B:44:0x00da] */
    /* JADX WARNING: Removed duplicated region for block: B:50:? A[RETURN, SYNTHETIC] */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    public final java.lang.String zzd(android.content.Context r12) {
        /*
            r11 = this;
            com.google.android.gms.internal.ads.zzavx r0 = r11.zzb
            r1 = 31
            java.lang.String r2 = "E"
            if (r0 == 0) goto L_0x0042
            com.google.android.gms.internal.ads.zzasp r3 = r11.zzc
            boolean r4 = r3.zzf()
            if (r4 != 0) goto L_0x0042
            com.google.common.util.concurrent.ListenableFuture r4 = r0.zza()
            if (r4 == 0) goto L_0x0042
            int r4 = android.os.Build.VERSION.SDK_INT
            if (r4 < r1) goto L_0x0024
            com.google.common.util.concurrent.ListenableFuture r4 = r0.zza()
            boolean r4 = r4.isDone()
            if (r4 == 0) goto L_0x0042
        L_0x0024:
            com.google.common.util.concurrent.ListenableFuture r0 = r0.zza()     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x0042 }
            int r3 = r3.zza()     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x0042 }
            long r3 = (long) r3     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x0042 }
            java.util.concurrent.TimeUnit r5 = java.util.concurrent.TimeUnit.MILLISECONDS     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x0042 }
            java.lang.Object r0 = r0.get(r3, r5)     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x0042 }
            com.google.android.gms.internal.ads.zzatq r0 = (com.google.android.gms.internal.ads.zzatq) r0     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x0042 }
            if (r0 == 0) goto L_0x0042
            boolean r3 = r0.zzaf()     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x0042 }
            if (r3 == 0) goto L_0x0042
            java.lang.String r0 = r0.zzg()     // Catch:{ InterruptedException | ExecutionException | TimeoutException -> 0x0042 }
            goto L_0x0043
        L_0x0042:
            r0 = r2
        L_0x0043:
            boolean r3 = r0.equals(r2)
            if (r3 == 0) goto L_0x00cc
            int r3 = android.os.Build.VERSION.SDK_INT
            if (r3 >= r1) goto L_0x004f
            goto L_0x00cc
        L_0x004f:
            int r3 = android.os.Build.VERSION.SDK_INT     // Catch:{ InterruptedException | ExecutionException -> 0x00cc }
            java.lang.String r4 = ""
            if (r3 >= r1) goto L_0x005a
            com.google.common.util.concurrent.ListenableFuture r1 = com.google.android.gms.internal.ads.zzgdn.zzh(r4)     // Catch:{ InterruptedException | ExecutionException -> 0x00cc }
            goto L_0x00be
        L_0x005a:
            java.lang.String r6 = r12.getPackageName()     // Catch:{ all -> 0x00ba }
            java.lang.String r1 = "X.509"
            java.security.cert.CertificateFactory r1 = java.security.cert.CertificateFactory.getInstance(r1)     // Catch:{ all -> 0x00ba }
            com.google.android.gms.internal.ads.zzgbd r3 = com.google.android.gms.internal.ads.zzgbd.zzi()     // Catch:{ all -> 0x00ba }
            com.google.android.gms.internal.ads.zzgbd r3 = r3.zzf()     // Catch:{ all -> 0x00ba }
            java.lang.String r5 = ""
            byte[] r3 = r3.zzk(r5)     // Catch:{ all -> 0x00ba }
            java.util.ArrayList r9 = new java.util.ArrayList     // Catch:{ all -> 0x00ba }
            r9.<init>()     // Catch:{ all -> 0x00ba }
            java.io.ByteArrayInputStream r5 = new java.io.ByteArrayInputStream     // Catch:{ all -> 0x00ba }
            r5.<init>(r3)     // Catch:{ all -> 0x00ba }
            java.security.cert.Certificate r3 = r1.generateCertificate(r5)     // Catch:{ all -> 0x00ba }
            r9.add(r3)     // Catch:{ all -> 0x00ba }
            java.lang.String r3 = android.os.Build.TYPE     // Catch:{ all -> 0x00ba }
            java.lang.String r5 = "user"
            boolean r3 = r3.equals(r5)     // Catch:{ all -> 0x00ba }
            if (r3 != 0) goto L_0x00a7
            com.google.android.gms.internal.ads.zzgbd r3 = com.google.android.gms.internal.ads.zzgbd.zzi()     // Catch:{ all -> 0x00ba }
            com.google.android.gms.internal.ads.zzgbd r3 = r3.zzf()     // Catch:{ all -> 0x00ba }
            java.lang.String r5 = ""
            byte[] r3 = r3.zzk(r5)     // Catch:{ all -> 0x00ba }
            java.io.ByteArrayInputStream r5 = new java.io.ByteArrayInputStream     // Catch:{ all -> 0x00ba }
            r5.<init>(r3)     // Catch:{ all -> 0x00ba }
            java.security.cert.Certificate r1 = r1.generateCertificate(r5)     // Catch:{ all -> 0x00ba }
            r9.add(r1)     // Catch:{ all -> 0x00ba }
        L_0x00a7:
            com.google.android.gms.internal.ads.zzauo r1 = new com.google.android.gms.internal.ads.zzauo     // Catch:{ all -> 0x00ba }
            r1.<init>()     // Catch:{ all -> 0x00ba }
            android.content.pm.PackageManager r5 = r12.getPackageManager()     // Catch:{ all -> 0x00ba }
            r7 = 0
            r8 = 8
            r10 = r1
            r5.requestChecksums(r6, r7, r8, r9, r10)     // Catch:{ all -> 0x00ba }
            com.google.android.gms.internal.ads.zzgeh r1 = r1.zza     // Catch:{ all -> 0x00ba }
            goto L_0x00be
        L_0x00ba:
            com.google.common.util.concurrent.ListenableFuture r1 = com.google.android.gms.internal.ads.zzgdn.zzh(r4)     // Catch:{ InterruptedException | ExecutionException -> 0x00cc }
        L_0x00be:
            java.lang.Object r1 = r1.get()     // Catch:{ InterruptedException | ExecutionException -> 0x00cc }
            java.lang.String r1 = (java.lang.String) r1     // Catch:{ InterruptedException | ExecutionException -> 0x00cc }
            boolean r3 = com.google.android.gms.internal.ads.zzfwg.zzd(r1)     // Catch:{ InterruptedException | ExecutionException -> 0x00cc }
            r4 = 1
            if (r4 == r3) goto L_0x00cc
            r0 = r1
        L_0x00cc:
            boolean r1 = r0.equals(r2)
            if (r1 == 0) goto L_0x00de
            com.google.android.gms.internal.ads.zzasp r1 = r11.zzc
            boolean r1 = r1.zzd()
            if (r1 != 0) goto L_0x00de
            java.lang.String r0 = zzc(r12)     // Catch:{ ClassCastException -> 0x00de }
        L_0x00de:
            return r0
        */
        throw new UnsupportedOperationException("Method not decompiled: com.google.android.gms.internal.ads.zzaup.zzd(android.content.Context):java.lang.String");
    }

    public final ListenableFuture zza() {
        return this.zza;
    }
}
